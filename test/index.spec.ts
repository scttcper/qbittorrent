...
it("should add torrent with savePath", async () => {
  ...
  await waitForTorrent(client);
  const torrentData = await client.getTorrent(
...
it("should get torrent trackers", async () => {
  ...
  expect(
    urls.includes("http://ipv6.torrent.ubuntu.com:6969/announce"),
  ).toBeTruthy();
});
it("should add torrent trackers", async () => {
  ...
  const torrentId = await setupTorrent(client);
  expect(
    await client.addTrackers(torrentId, "http://tracker.example.com/announce"),
  ).toBeTruthy();
  const trackers = await client.torrentTrackers(torrentId);
  expect(trackers.map((x) => x.url)).includes(
    "http://tracker.example.com/announce",
  );
  expect(
    await client.removeTrackers(
      torrentId,
      "http://tracker.example.com/announce",
    ),
  ).toBeTruthy();
  const trackers2 = await client.torrentTrackers(torrentId);
  expect(trackers2.map((x) => x.url)).not.includes(
    "http://tracker.example.com/announce",
  );
});
...
it("should get the default save path", async () => {
  const client = new QBittorrent({ baseUrl, username, password });
  const savePath = await client.getDefaultSavePath();
